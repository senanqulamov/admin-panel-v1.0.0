        ...
        <div class="container-fluid d-flex align-items-center justify-content-between flex-wrap flex-sm-nowrap">
            ...
            <div class="d-flex align-items-center flex-wrap mr-1">
                ...
                <div class="d-flex align-items-baseline flex-wrap mr-5">
                    <!--begin::Page Title-->
                    <h5 class="text-dark font-weight-bold my-1 mr-5">Məhsullar Kateqoriyalar Axtar</h5>
                    <!--end::Page Title-->
                    <!--begin::Breadcrumb-->
            ...
            <div class="card card-custom gutter-b">
                <div class="card-header">
                    <div class="card-title">
                        <h3 class="card-label">Məhsullar Kateqoriyalar</h3>
                    </div>
                    <div class="card-toolbar">
                        <div class="card-title">
                            <form action="{{ route('admin.product.category.search') }}" method="GET">
                        ...
                        <!--  ADD BUTTON  -->
                        <a href="{{ route('admin.product.category.add') }}">
                            <button
                                tooltip="Əlavə et"
                                flow="left"
                ...
                <div class="card-body">
                    ...
                    <table class="table table-hover table-striped" data-sorting="true">
                        ...
                        @foreach($productCategories as $key => $productCategory)
                            ...
                            <tr class="table-id-{{ $productCategory->id }}" data-index="{{ $productCategory->id }}" data-position="{{ $productCategory->sort }}">

                                <!-- SELECT ALL -->
                                <td>
                                    <label class="checkbox checkbox-success select-element-btn" data-id="{{ $productCategory->id }}">
                                        <input type="checkbox"   />
                                        <span></span>
                                    </label>
                                </td>
                                ...
                                <!--  NAME  -->
                                <td>
                                    {{ \App\Services\CategoriesService::getTreeProductIndex($defaultLanguage,$productCategory->id,$productCategory->parent) }}
                                    {{--                                    <a href="{{ route('admin.product.category.edit',$productCategory->id) }}">{{ $productCategory->productsCategoriesTranslations[0]->name }}</a>--}}
                                </td>

                                <!--  SAY  -->
                                <td><a href="{{ route('admin.product.categories',$productCategory->id) }}">{{ count($productCategories[$key]->getProductsCount) }}</a></td>


                                <!--  Tarix  -->
                                <td>{{ updateDate($productCategory->updated_at,$productCategory->productsCategoriesTranslations) }}</td>
                                ...
                                <td width="100">
                                        ...
                                        <label>
                                            <input
                                                ...
                                                type="checkbox"
                                                {{ $productCategory->status == 1? 'checked="checked"':"" }}
                                                name="select">
                                            <span></span>
                                        </label>
									</span>
                                        ...
                                        <div class="dropdown-menu p-0 m-0 dropdown-menu-md dropdown-menu-right"
                                            ...
                                            <ul class="navi navi-hover">
                                                ...
                                                <li
                                                    ...
                                                    <a href="{{ route('admin.product.category.edit',$productCategory->id) }}" class="navi-link text-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center flex-wrap">
                        <div class="d-flex align-items-center py-3">
                            <span class="text-muted">
                                Cəmi <b><span class="totalCount">{{ $productCategories->total() }}</span></b> yazı
                                @if($productCategories->hasPages())
                                    , <b>{{ $productCategories->lastPage() }}
                                </b> səhifədən  <b>{{ $productCategories->currentPage() }}</b>-ci göstərildi.
                                @endif
...
@section('js')
    ...
    <script>
        $(function () {
            ...
            $(document).on('click', '.statusActive', function () {
                ...
                $.ajax({
                    url: "{{ route('admin.product.category.statusAjax') }}",
                    type: 'POST',
        ...
        $(document).on('click', '.deleteButton', function () {
            ...
            $.ajax({
                url: "{{ route('admin.product.category.deleteAjax') }}",
                type: 'POST',
                data: {id: dataID},
                dataType: 'JSON',
                success: function (response) {
                    if (response.success) {
                        ...
                        if(response.error == null){
                            ...
                            Swal.fire({
                                title: "Diqqət!",
                                html: "Bu kateqoriyanı silmək olmaz! <b>" + response.name + "</b> kateqoriyasına aid məhsullar var!",
                                icon: "error",
    ...
    <script>
        deleteALlSelectedElements(
            ...
            'Sil!',
            'Xeyir',
            '{{ route('admin.product.category.allDeleteAjax') }}',
